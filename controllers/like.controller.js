...
const getLikedPost = async (req, res) => {
    ...
    try {
        ...
        const allLikedPosts = await Like.aggregate([
            ...
            { $unwind: "$likedPosts" }, // Flatten the likedPosts array
            {
                $lookup: {
                    from: "comments",
                    localField: "likedPosts._id",
                    foreignField: "postId", // Assuming postId in comments refers to the post
                    as: "likedPosts.comments"
                }
            },
            {
                $group: {
        ...
        // console.log(allLikedPosts.length);
        res.status(200).json(new ApiResponse(200, {allLikedPosts,username:user?.name}, "Liked posts fetched successfully"))
